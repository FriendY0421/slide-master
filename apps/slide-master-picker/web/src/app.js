...
function presetCard(p) {
  return `<button type="button" class="preset-card${selectedPreset?.id === p.id ? " selected" : ""}" data-preset="${esc(p.id)}">
    <strong>${esc(p.display_name)}</strong><span>${esc(p.summary)}</span>
    <small>${esc(p.slide_range)}장 · 텍스트 ${esc(p.visual_ratio?.text)}% / 시각 ${esc(p.visual_ratio?.visual)}%</small>
  </button>`;
}

function renderGallery() {
  const list = mode === "all" ? payload.all_templates : payload.shortlist;
  root.innerHTML = `<section class="picker-shell">
    ...
    <div class="grid">${list.map(templateCard).join("")}</div>
    <div class="free"><button type="button" data-free>Free Design 선택</button></div>
  </section>`;
  bind("[data-mode]", (el) => { mode = el.dataset.mode; persistState(); renderGallery(); });
...
const finalToken = () => `${selectedTemplate.id} | preset:${selectedPreset.id}`;

async function confirmSelection() {
  const token = finalToken();
  const message = `PPT 템플릿 최종 선택: ${token}. 이 선택을 확정하고 Slide Master 게이트에 기록한 뒤 PPT 제작을 계속 진행해줘.`;
  try {
