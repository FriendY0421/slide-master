...
> **FriendY canonical preflight supersedes the generic step order for new decks.** Before project initialization: (1) show/select template, (2) show/select production preset, (3) research/verify evidence, (4) present the full slide-by-slide preview with title + core message + content points + visual/layout plan, (5) let the user revise slide count/order/content (20/30+ is valid when evidence supports it), and (6) record the explicitly approved current revision with `storyline_gate.py`. Only then run `new_deck_init.py`. This prevents the historical failure where slide authoring started before the user saw the planned content.
...
```bash
python3 ${SKILL_DIR}/scripts/new_deck_init.py <project_name> --format <format> --template-selection-result <template_result.json> --storyline-approval-result <storyline_approval.json>
```

**Fail-closed new-deck entry:** `<template_result.json>` must come from the mandatory template + production-preset selection gate, and `<storyline_approval.json>` must represent the exact slide-by-slide preview revision explicitly approved by the user. A bare `project_manager.py init` is not a valid new-deck entry. For FriendY new decks, template/preset selection ? research ? editable storyline preview/approval occurs before this initialization command; the older generic Step numbering must not be used to bypass that preflight. Direct-PPTX/resume workflows use their own route and, when they hand off to shared SVG export, record a documented exemption with `template_gate.py exempt`.
...
**Production preset is execution input, not a label.** Read `template_selection.json production_preset_profile` before Strategist typography/layout planning. Its `delivery_purpose`, `body_px`, `content_density`, and `visual_ratio` seed the plan unless the user explicitly overrides them. Do not silently revert to pre-2026-08-30 small-type defaults.
...
**Lightweight exported-PPTX visual sanity check (DEFAULT ? one pass, not deep verification)**: Read the `_pptx_render/<stem>-grid.png` once for obvious text overflow/collision, missing assets, font substitution, **cramped card text, visibly tight line spacing, near-zero card padding, oversized unused outer margins, weak text hierarchy, or tiny decorative visuals floating in empty space**. A slide is not clean merely because nothing overlaps; the composition must look intentionally filled and readable at normal presentation zoom.
