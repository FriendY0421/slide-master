...
**이 템플릿의 본문 baseline은 `17`이다.** Strategist는 확인 단계(Stage 2)에서 `delivery_purpose`의 일반 기본값(`text` 24 / `balanced` 30 / `presentation` 36)이 아니라 **아래 네이티브 램프를 추천값으로 제시**해야 한다. 사용자가 명시적으로 다른 값을 확정하면 그 값이 이긴다(확정값 우선 규칙은 그대로).
