...
category: brand
summary: 코발트 데이터 브리핑 덱 — KPI, 성과, 실적, 수치 중심 보고
...
| Identity green | `#2563EB` | `--naver-green` | Core segment title/KPI, key table rows, highlight boxes, current-quarter box |
...
- **`#5BDB7D` is a surface, not an accent** — cover/closing background only; accents on white always use `#2563EB`
...
실적요약/비용·손익/Appendix형: title + 1–2 intro 개조식 bullets + full-width financial table (header band `#F5F5F5`, hairline rows, green bold key rows, right-aligned tabular numerals, italic Y/Y·Q/Q columns, 2px `#2563EB` box around the current-quarter column group) + bottom-left footnote slot.
...
- Key rows (매출액/영업비용/영업이익) bold `#2563EB`; indent hierarchy with 1-level (segment) and 2-level ("- 인건비") rows
- Numerals right-aligned tabular; delta columns (Y/Y, Q/Q) italic; ratio rows ("영업이익률 (%)") italic gray
- **2px `#2563EB` box around the current-quarter column group** (value + deltas) — the signature highlight
