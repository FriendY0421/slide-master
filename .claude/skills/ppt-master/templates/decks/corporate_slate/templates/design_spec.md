---
deck_id: corporate_slate
kind: deck
native_structure_mode: structured
category: brand
summary: 슬레이트 기업 보고 덱 — 업무보고, 공지, 현황 공유, 운영 브리핑
...
| Identity green | `#334155` | `--naver-green` | Core segment title/KPI, key table rows, highlight boxes, current-quarter box |
...
- **`#5BDB7D` is a surface, not an accent** — cover/closing background only; accents on white always use `#334155`
...
실적요약/비용·손익/Appendix형: title + 1–2 intro 개조식 bullets + full-width financial table (header band `#F5F5F5`, hairline rows, green bold key rows, right-aligned tabular numerals, italic Y/Y·Q/Q columns, 2px `#334155` box around the current-quarter column group) + bottom-left footnote slot.
...
- Key rows (매출액/영업비용/영업이익) bold `#334155`; indent hierarchy with 1-level (segment) and 2-level ("- 인건비") rows
- Numerals right-aligned tabular; delta columns (Y/Y, Q/Q) italic; ratio rows ("영업이익률 (%)") italic gray
- **2px `#334155` box around the current-quarter column group** (value + deltas) — the signature highlight
