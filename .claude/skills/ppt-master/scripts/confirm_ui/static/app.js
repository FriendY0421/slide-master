...
(function () {
    ...
    function deliveryBodyPx(purposeId) {
        if (purposeId === "text") return { lo: 26, hi: 30, def: 28 };
        if (purposeId === "presentation") return { lo: 38, hi: 44, def: 40 };
        return { lo: 32, hi: 36, def: 34 }; // balanced — the default
    }
    ...
    function renderStylePreview(host) {
        ...
        function paint() {
            ...
            var rawSize = parseFloat(typ.body_size) || (isPptCanvas(STATE.canvas) ? 34 : 18);
