...
- The picker app is read-only and is the canonical ChatGPT/GPTS interactive selection surface. Its final button sends the selected template/preset back into chat; treat that returned user message as the explicit selection confirmation.
...
`request → FAH → Slide Master Picker app → live catalog → interactive template detail → production preset → final button/user message → picker/selection evidence → generation → QA → PPTX`
